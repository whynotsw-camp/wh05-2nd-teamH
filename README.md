...
김민지
데이터 시각화 및 유사도 분석을 능숙하게 수행, 상관관계 분석 등 통계적 기법을 프로젝트에 잘 녹여냄

서동겸
웹 스크래핑 기반 데이터 수집에 큰 기여

웹 구조 변화에 유연하게 대응할 수 있는 수집 전략 고안

김찬호
데이터 처리를 원할하게 할 모델링 고안

이성재
어제 지각 안 함 (프로젝트 내 기적의 사건)

찹쌀고로케 맛집 공유로 팀워크에 기여

2.2 개선이 필요한 점 (What could be improved)
프로젝트 과정에서 비효율적이었거나 개선이 필요한 부분

김민지
keyBert 모델 사용하긴 했는데, 여전히 튜닝이 더 필요하며 모델에 있어서 Multilingual Embedding 모델 활용 및 LLM 활용해서 더 높은 정확도의 전처리와 유사도 계산을 할 예정
...
김찬호
모델링 과정에서 전문 지식의 많은 부족함을 느낌
더 구체적인 일정 관리가 있었다면 활동이 더 원활했을 것

이성재
찹쌀고로케 너무 늦게 추천함 (팀 사기 저하 원인 중 하나)

2.3 배운 점 (Lessons learned)
이번 프로젝트를 통해 새롭게 학습한 내용

김민지
통계 기법 외에도 LLM 및 Multilingual Embedding 활용 가능성 체득

키워드 기반 유사도 계산에 있어 다양한 접근법 시도

서동겸
데이터 엔지니어 역할에 대한 실무적 감각 획득

DOM 변화 대응 전략, OCR 도구 비교 등 실전 기술 학습

김찬호
핵심 키워드 추출 및 임베딩에 필요한 기술 습득

자연어 처리 기반 전처리 기술에 대한 이해도 증가

이성재
빽다방 고구마빵은 일찍 가야 맛볼 수 있음 (실패에서 얻은 교훈)

2.4 다음 단계 (Action items)
향후 프로젝트에 적용할 개선 방안 또는 실행 계획

🧑‍💻 김민지
🔧 LLM 기반 전처리 강화 및 다국어 임베딩 모델 실험

🧪 모델 정확도 향상을 위한 키워드 추출 파이프라인 개선

👨‍💻 서동겸
🔁 스크래핑 코드의 모듈화 및 변화 감지 자동화 시스템 구축

🔍 OCR 라이브러리 성능 테스트 및 적재 전략 개선

👨‍💼 김찬호
🛠 전처리 자동화 도구 투입 검토

📌 추출된 키워드를 임베딩 후 직무 추천 시스템과 연동

🧍‍♂️ 이성재
🌶 월요일 닭발 약속 사수 (팀 사기 및 결속력 증진 핵심 전략)
